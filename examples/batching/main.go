...
func newPipeline(withDrawer bool) (*pipeline.Pipeline, error) {
	...
	drw := drawer.NewSVGDrawer("examples/batching/pipeline.dot")
...
func main() {
	drawerEnabled := flag.Bool("drawer", false, "write examples/batching/pipeline.dot with metrics")
	flag.Parse()
	...
	sink := pipeline.Sink(pipe, "print", batch, func(ctx context.Context, input []int) error {
		fmt.Printf("batch: %v\n", input)
		return nil
	})
	if sink == nil {
